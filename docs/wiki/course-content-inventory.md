...
- **A title is printed once, by exactly one thing.** Three places used to restate it: the leading H1 (stripped, above); a callout's bold first line, which `parse.mjs` *lifts* into `label` and removes from `text` — never copies, so a renderer printing both no longer doubles it; and a short `##` stub sitting directly above a `###` that said the same thing (13 of these in m1 teaching lessons, collapsed into one descriptive `##` to match m2–m5). Both remaining shapes are guarded below.
...
Two more guard the "printed once" rule above: a callout whose body's first line repeats its own `label`, and a heading immediately restated by a deeper heading (equal, or the parent as a word-prefix of the child — `## Tokens` / `### Tokens: the unit of…`). A deeper heading that genuinely subdivides its parent is untouched, e.g. m2-multimodal's `## Different ways to send an image` / `### Inline base64`.
