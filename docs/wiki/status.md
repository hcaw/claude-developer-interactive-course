...
- **Verified locally**: all 108 lesson pages resolve (`/sections/*` is gone); no answer-key text appears in any client bundle or in served HTML; 43 unit tests cover completion derivation, quiz grading, the manifest's shape and video mapping, and the domain rule; 16 integration tests cover the heartbeat upsert and access control against a real Postgres.
